...
$.Widget.prototype = {
	...
	_createWidget: function( options, element ) {
		...
		this.udgd = widgetUdgd++;
		this.eventNamespace = "." + this.widgetName + this.udgd;
	...
	_hoverable: function( element ) {
		...
		this._on( element, {
			mouseenter: function( event ) {
				this._addClass( $( event.currentTarget ), null, "dg-state-hover" );
			},
			mouseleave: function( event ) {
				this._removeClass( $( event.currentTarget ), null, "dg-state-hover" );
			}
		} );
	},
	...
	_focusable: function( element ) {
		...
		this._on( element, {
			focusin: function( event ) {
				this._addClass( $( event.currentTarget ), null, "dg-state-focus" );
			},
			focusout: function( event ) {
				this._removeClass( $( event.currentTarget ), null, "dg-state-focus" );
			}
		} );
	},
...
var widgetsMouse = $.widget( "dg.mouse", {
	version: "1.12.1",
	options: {
...
$.widget( "dg.draggable", $.dg.mouse, {
	...
	_mouseUp: function( event ) {
		...
		return $.dg.mouse.prototype._mouseUp.call( this, event );
	},
	...
	_trigger: function( type, event, dg ) {
		...
		return $.Widget.prototype._trigger.call( this, type, event, dg );
	},
...
$.dg.plugin.add( "draggable", "cursor", {
	...
	stop: function( event, dg, instance ) {
...
$.dg.plugin.add( "draggable", "opacity", {
	...
	stop: function( event, dg, instance ) {
		var o = instance.options;
		if ( o._opacity ) {
			$( dg.helper ).css( "opacity", o._opacity );
		}
	}
} );
...
$.dg.plugin.add( "draggable", "scroll", {
	...
	drag: function( event, dg, i  ) {
		...
		if ( scrolled !== false && $.dg.ddmanager && !o.dropBehaviour ) {
			$.dg.ddmanager.prepareOffsets( i, event );
		}
...
$.dg.plugin.add( "draggable", "snap", {
	...
	drag: function( event, dg, inst ) {

		var ts, bs, ls, rs, l, r, t, b, i, first,
			o = inst.options,
			d = o.snapTolerance,
			x1 = dg.offset.left, x2 = x1 + inst.helperProportions.width,
			y1 = dg.offset.top, y2 = y1 + inst.helperProportions.height;
		...
		for ( i = inst.snapElements.length - 1; i >= 0; i-- ) {
				...
				if ( inst.snapElements[ i ].snapping ) {
					( inst.options.snap.release &&
						inst.options.snap.release.call(
							inst.element,
							event,
							$.extend( inst._dgHash(), { snapItem: inst.snapElements[ i ].item } )
						) );
				}
			...
			if ( o.snapMode !== "inner" ) {
				...
				rs = Math.abs( r - x1 ) <= d;
				if ( ts ) {
					dg.position.top = inst._convertPositionTo( "relative", {
				...
				if ( ls ) {
					dg.position.left = inst._convertPositionTo( "relative", {
						top: 0,
...
$.dg.plugin.add( "draggable", "zIndex", {
	...
	stop: function( event, dg, instance ) {
		...
		if ( o._zIndex ) {
			$( dg.helper ).css( "zIndex", o._zIndex );
		}
	}
} );
